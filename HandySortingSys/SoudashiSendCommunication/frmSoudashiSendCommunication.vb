...
Public Class frmSoudashiSendCommunication
  ...
  Private Function SqlSelTrnSoudashiTanaSelect() As String
    ...
    sql &= "      , CASE WHEN (MIN(TORIKOMI_JOKYO_FLG) = " & CInt(SHUKKA_STATUS.SOUDASHI_SOUSINZUMI) & " Or MAX(SOUDASHI_RECEIVE_DATE) IS NULL) THEN '未' ELSE '済' END AS TORIKOMI_JOKYO_FLG "
  ...
  Private Function SqlSelTrnSoudashi(prmTanaList As List(Of String)) As String
    ...
    sql &= "      ,CASE "
    sql &= "        WHEN MST_ITEM.TANKA_TANI IS NULL    THEN MAX(TRN_SHUKKA.HACHU_TANI) "
    sql &= "        WHEN TRIM(MST_ITEM.TANKA_TANI) = '' THEN MAX(TRN_SHUKKA.HACHU_TANI) "
