...
Err_Exit:
  ...
  ''' 当月末日の日付を取得
  ''' </summary>
  ''' <returns>当月末日の日付文字列（yyyy/MM/dd形式）</returns>
  Public Shared Function ComGetEndOfMonth() As String
    Dim tmpFirstDate As String = ComGetProcDate().Substring(0, 7) & "/01"                       ' 当月初日
    Dim tmpNextMonth As String = Date.Parse(tmpFirstDate).AddMonths(1).ToString("yyyy/MM/dd")   ' 来月初日
    Dim tmpEndOfMonth As String = Date.Parse(tmpNextMonth).AddDays(-1).ToString("yyyy/MM/dd")   ' 当月末日

    Return tmpEndOfMonth
  End Function

  ''' <summary>
  ''' 本日日付の数年後を取得
  ''' </summary>
  ''' <param name="prmAddValue">加算する年数（マイナスも可）</param>
  ''' <returns>日付文字列（yyyy/MM/dd形式）</returns>
  Public Shared Function ComGetAddingYear(prmAddValue As Long) As String
    Return Date.Parse(ComGetProcDate()).AddYears(prmAddValue).ToString("yyyy/MM/dd")
  End Function
  '-------------------------------------------------
  ' 関数名：ComGetProcTime
  ...
  Public Shared Function ComGetProcTime() As String
    ...
    'Try
    '  Call mainDatabase.GetResult(tmpDt, "SELECT GETDATE() ")
    'Catch ex As Exception
    '  Call ComWriteErrLog(ex)
  ...
  ''' <param name="prmEncode">Encode</param>
  Public Shared Sub ComWriteLog(ByVal desc As String, ByVal filePath As String, Optional prmEncode As Encoding = Nothing)
    Dim strWr As System.IO.StreamWriter

    If prmEncode Is Nothing Then
      strWr = New System.IO.StreamWriter(filePath, True)
    Else
      strWr = New System.IO.StreamWriter(filePath, True, prmEncode)
    End If
  ...
  Public Shared Sub ComWriteErrLog(ByVal ex As Exception,
    ...
    If filePath = "" Then filePath = GetLogFilePath("ERROR")
    '---
  ...
  ''' 文字列をバイト数指定で切り出し
  ''' </summary>
  ''' <param name="strInput">対象文字列</param>
  ''' <param name="intLen">バイト数</param>
  ''' <returns>切り出した文字列</returns>
  ''' <remarks>
  '''   参考) https://tex2e.github.io/blog/vb.net/cut-str-byte-length
  ''' </remarks>
  Public Shared Function CutStrByteLen(ByVal strInput As String, ByVal intLen As Integer) As String
  ...
  Public Shared Sub SetRegistry(prmKey As String, prmName As String, prmStr As String)

    Dim regkey As Microsoft.Win32.RegistryKey =
            Microsoft.Win32.Registry.CurrentUser.CreateSubKey(prmKey)

    'レジストリに書き込み
    '文字列を書き込む（REG_SZで書き込まれる）
    regkey.SetValue("string", prmStr)
    ''整数（Int32）を書き込む（REG_DWORDで書き込まれる）
    'regkey.SetValue(prmName, prmInt)
  ...
  ''' <param name="prmName"></param>
  ''' <returns></returns>
  Public Shared Function GetRegistry(prmKey As String, prmName As String) As Integer

    'キー（HKEY_CURRENT_USER\Software\test\sub）を読み取り専用で開く
    Dim regkey As Microsoft.Win32.RegistryKey =
            Microsoft.Win32.Registry.CurrentUser.OpenSubKey(prmKey, False)
    'キーが存在しないときは null が返される
    If regkey Is Nothing Then
      SetRegistry(prmKey, prmName, 0)
      Return ""
    End If

    '文字列を読み込む
    '指定した名前の値が存在しないときは null が返される
    Dim intValue As Integer = DirectCast(regkey.GetValue(prmName), Integer)
  ...
  '''   false : 出力対象無し
  ''' </returns>
  Public Shared Function Sql2CSV(prmSql As String, prmCsvFilePath As String, Optional ByRef prmOutPutCount As Integer = 0) As Boolean
    ...
    Try
      ...
      If tmpDt.Rows.Count <= 0 Then
        Return False
      Else
        prmOutPutCount = tmpDt.Rows.Count
        ' カラム名称取得
        For Each tmpCol As DataColumn In tmpDt.Columns
          tmpKeyList.Add(tmpCol.ColumnName)
        Next
        ...
        ' 全データをループ
        For Each tmpCurrentRow As DataRow In tmpDt.Rows
          For Each tmpKeyName As String In tmpKeyList
  ...
  Public Shared Function GetLogFilePath(prmLogType As String) As String
    Dim tmpFileName As String = String.Empty
    Dim tmpDirPath As String = System.IO.Directory.GetCurrentDirectory() & "\..\LOG\"


    Select Case prmLogType.ToUpper
      Case "ERROR".ToUpper
        tmpFileName = "ERROR.txt"
        tmpDirPath &= "ERROR\"
      Case "DELETE".ToUpper
        tmpFileName = "_DELETE.CSV"
        tmpFileName = Date.Now().ToString("yyyyMMddHHmmss") & tmpFileName
        tmpDirPath &= "DELETE\"
      Case "CSV".ToUpper
        tmpFileName = "_WEIGHTLOG.CSV"
        tmpFileName = Date.Now().ToString("yyyyMMddHHmmss") & tmpFileName
        tmpDirPath &= "RESULT\"
      Case Else
        Throw New Exception("ログ出力パラメータエラー")
    End Select

    ' 指定したフォルダがあるかどうか確認する
    If False = System.IO.Directory.Exists(tmpDirPath) Then
      ' 指定したフォルダ名を作成する
      System.IO.Directory.CreateDirectory(tmpDirPath)
    End If

    Return (tmpDirPath & tmpFileName)
  End Function
